...
PredType  BPNotTaken::predict(const Instruction * inst, InstID oracleID, bool doUpdate)
{
 ...
 //Code for project
   ...
   if (mInstructionMap.find(inst->getAddr()) == mInstructionMap.end()) {
       oneToTen = oneToTen + 1;
       mInstructionMap[inst->getAddr()] = 1;
   } else {
        count =  mInstructionMap[inst->getAddr()] + 1;
	mInstructionMap[inst->getAddr()] = mInstructionMap[inst->getAddr()] + 1;
	if (count == 10 ) {
	    oneToTen = oneToTen - 1;
	    tenToHundred = tenToHundred + 1;
	} else if (count == 100) {
	    tenToHundred = tenToHundred - 1;
	    oneHundredToThousand += 1;
	} else if (count == 1000) {
	    oneHundredToThousand = oneHundredToThousand - 1;
	    moreThanThousand += 1;
        } 
    }
    
    bool iscorrect = (inst->calcNextInstID() == oracleID);
    updateStats(inst->getAddr(), count, !iscorrect);
    
    return iscorrect ? CorrectPrediction : MissPrediction;
}

void BPNotTaken::updateStats(int32_t instructionId, int currentCount, bool incorrect) 
{

    	    
    	    if (currentCount < 10 ) {
		 if (!incorrect)
		     oneToTenCorrect[instructionId] = oneToTenCorrect[instructionId] + 1;
		 else
		     oneToTenIncorrect[instructionId] = oneToTenIncorrect[instructionId] + 1;
	    } else if (currentCount > 10 && currentCount < 100) {
		 if (!incorrect)
		     tenToHundredCorrect[instructionId] = tenToHundredCorrect[instructionId] + 1;
		 else
		     tenToHundredIncorrect[instructionId] = tenToHundredIncorrect[instructionId] + 1;
	    } else if (currentCount > 100 && currentCount < 1000) {
		if (!incorrect)
		    oneHundredToThousandCorrect[instructionId] = oneHundredToThousandCorrect[instructionId] + 1;
		else
		    oneHundredToThousandIncorrect[instructionId] = oneHundredToThousandIncorrect[instructionId] + 1; 
	    } else if (currentCount > 1000) {
		if (!incorrect)
		    moreThanThousandCorrect[instructionId] = moreThanThousandCorrect[instructionId] + 1;
		else
		    moreThanThousandIncorrect[instructionId] = moreThanThousandIncorrect[instructionId] + 1;
	    } 
	    
	    if (currentCount == 10 ) {
	    	if (!incorrect) {
	    		tenToHundredCorrect[instructionId] = oneToTenCorrect[instructionId] + 1;
	    		tenToHundredIncorrect[instructionId] = oneToTenIncorrect[instructionId];
	    	} else {
	    		tenToHundredCorrect[instructionId] = oneToTenCorrect[instructionId];
	    		tenToHundredIncorrect[instructionId] = oneToTenIncorrect[instructionId] + 1;
	    	}
	    	 
		 oneToTenCorrect.erase(instructionId);
		 oneToTenIncorrect.erase(instructionId);
	    } else if (currentCount == 100) {
	    	if (!incorrect) {
	    		oneHundredToThousandCorrect[instructionId] = tenToHundredCorrect[instructionId] + 1;
	    		oneHundredToThousandIncorrect[instructionId] = tenToHundredIncorrect[instructionId];
	    	} else {
	    		oneHundredToThousandCorrect[instructionId] = tenToHundredCorrect[instructionId];
	    		oneHundredToThousandIncorrect[instructionId] = tenToHundredIncorrect[instructionId] + 1;
	    	}
	    	 
		 tenToHundredIncorrect.erase(instructionId);
		 tenToHundredCorrect.erase(instructionId);
	    } else if (currentCount == 1000) {
	    	if (!incorrect) {
	    		moreThanThousandCorrect[instructionId] = oneHundredToThousandCorrect[instructionId] + 1;
	    		moreThanThousandIncorrect[instructionId] = oneHundredToThousandIncorrect[instructionId];
	    	} else {
	    		moreThanThousandCorrect[instructionId] = oneHundredToThousandCorrect[instructionId];
	    		moreThanThousandIncorrect[instructionId] = oneHundredToThousandIncorrect[instructionId] + 1;
	    	}
	    	 
		 oneHundredToThousandCorrect.erase(instructionId);
		 oneHundredToThousandIncorrect.erase(instructionId);
	    } 
}
...
PredType BPHybrid::predict(const Instruction *inst, InstID oracleID, bool doUpdate)
{
   ...
   if (mInstructionMap.find(inst->getAddr()) == mInstructionMap.end()) {
    ...
    if( inst->isBranchTaken() ) {
    	PredType prediction = btb.predict(inst, oracleID, doUpdate);
    	//Unconditional branch, count always as correct
    	updateStats(inst->getAddr(), count, false);
        return prediction;
    }
    ...
    if (taken != ptaken) {
    	updateStats(inst->getAddr(), count, true);
        if (doUpdate)
            btb.updateOnly(inst,oracleID);
        return MissPrediction;
    }
    ...
    if (ptaken && prediction == MissPrediction)
    	    updateStats(inst->getAddr(), count, true);
    else
    	    updateStats(inst->getAddr(), count, false);
...
void BPHybrid::updateStats(int32_t instructionId, int currentCount, bool incorrect) 
{
